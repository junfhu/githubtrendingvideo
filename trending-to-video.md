...
description: 从 GitHub Trending 热门项目中选择一个，自动生成 Remotion 宣传视频（截图 + 动画 + Edge TTS 语音旁白）
...
4. **编辑** → 左侧截图预览，右侧项目信息 + 功能列表 + S1-S6 场景旁白文本（每行可编辑）
5. **Generate Voice** → 通过 Edge TTS（`zh-CN-YunxiNeural`）生成中文语音
6. **Preview in Remotion** → 自动生成语音后打开 Remotion Studio（视频时长匹配语音）
7. **Build Video** → 渲染最终 MP4

## 场景与时间线

视频共 6 个场景，时长由语音自动决定（`durationSeconds * 30 + 15` 帧）。场景时间与旁白文本对齐（`narrationTiming` 中的字符位置比例）。
...
| S5 项目介绍 | **有demo图片时**：ken-burns轮播 + 项目描述卡片；**无demo图片时**：核心价值截图平移 + 项目简介覆盖层；**无截图时**：精美介绍卡展示项目能力 | demoImages 或 intro | 项目核心价值描述 |
| S6 结尾 | 紫蓝渐变 + "关注我，获得最新的实用项目信息" | — | 结尾语音 |
...
- **S6**：与 S1 呼应的紫蓝渐变背景 + 磨砂玻璃卡片，白色文字居中

### 演示图片提取（S5 增强）
`extract_demo_images()` 从 README 中提取项目输出截图/演示图片：
1. 扫描 README 中的 `![alt](url)` 和 `<img src="url">` 图片
2. 优先从 Demo/Example/Screenshot/效果 等章节提取（score=10），其次从 Usage/Getting Started 等章节（score=5）
3. 跳过 badges/shields.io 等小图标（<5KB 或 <200px 宽）
4. 下载最多 5 张图片到 `remotion/public/demo_<repo>_<index>.png`
5. 大图自动缩放至 1920px 宽度

**S5 有 demo 图片时**：ken-burns 缩放 + 平移动画轮播展示，带进度指示器和"DEMO OUTPUT"标签
**S5 无 demo 图片时**：回退为原有的 README 截图滚动模式
...
### 场景时间对齐
**S1-S4、S6**：固定 2.5 秒。**S5**：按字符比例分配音频剩余时间。`sceneDurations` 保存在 props.json，`buildScenes()` 将其转为精确帧数。

### 语音生成
使用 **Edge TTS**（`zh-CN-YunxiNeural`），免费云端服务，无需本地模型或 GPU。`normalize_for_tts()` 自动转换全大写词（如 `SKILL.md`→`skill markdown`）。可通过环境变量 `EDGE_TTS_VOICE` 切换语音。
...
interface VideoProps {
  ...
  demoImages: string[];   // demo image filenames in public/ (extracted from README)
